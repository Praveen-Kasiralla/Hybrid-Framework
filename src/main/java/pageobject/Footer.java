...
public class Footer {
	...
	public void clickHomeLoan() throws IOException, InterruptedException {
		exReport.enterInfoLog("Clicking on Home Loan");
		logs.enterInfoLog("Clicking On Home Loan");
		Thread.sleep(1000);
		webDriver.clickElement(driver, FooterUi.homeLoan);
		webDriver.switchTab(driver, 1);
		webDriver.waitUntilPageTitle(driver, "Loans");
		if(webDriver.getCurrentUrl(driver).contains("Loans")) {
			exReport.enterPassLogWithSnap("Home Loan has opened");
			logs.enterInfoLog("Home Loan has opened");
		}else {
			exReport.enterFailLogWithSnap("Home Loan didn't open");
			logs.enterErrorLog("Home Loan didn't open");
		}
	}
}
